...
grimmTalesPath = os.path.join(workingDir, 'conspiracyText')
...
def clean_doc(doc):
    try:
        ...
        text = open(doc, 'r', encoding='ansi').read()
    # text = codecs.open(doc, 'r', encoding='utf-8').read()
    # with codecs.open(doc, 'r', encoding='utf8') as file:
    #     lines = file.read()
...
lda_model = LdaModel(corpus=corpus, id2word=id2word, num_topics=40)
